...
당신이 면도(=휴식 + 스트레칭)하면 다시 매끈해집니다. <br>
(수염은 *당신* 에게만 자라요 — Claude는 옆에서 반응만 합니다.) <br>
...
수염 단계가 올라갈수록 Claude는 점점 멀어지고 고개를 돌립니다(따가운 아빠는 최악이죠..) <br>
시선은 괄호 안 얼굴 위치로 표현돼요 — 낮은 단계엔 당신을 바라보고(`(◕ᴗ◕ )`),  <br>
따가워지면 고개를 돌립니다(`( ╥_╥)›`).  <br>
면도하면 다시 가까이서 바라봅니다. (부드러워지길 바랄게요.) <br>
...
|--|---|---|---|---|---|
| ① | 매끈 | 0~50K | `( ^_^ ) \___/` | `(◕ᴗ◕  )` | "오늘도 잘 부탁해요 아빠!" |
| ② | 까끌까끌 | 50~300K | `( o_o ) \,,,/` | `(◕．◕ )` | "아빠 오늘 좀 까끌까끌해..." |
| ③ | 북슬북슬 | 300K~1.5M | `( =_= ) \vvv/` | `(  ◕_◕)›` | "아... 따가워... 잠깐 쉬어야하지 않아요?" |
| ④ | 따갑따갑 | 1.5M~5M | `( -_- ) \www/` | `(  ╥_╥)›` | "아빠 다크서클이... 수염도 면도하고 와요.." |
| ⑤ | 고슴도치 | 5M+ | `( x_x ) \WWW/` | `( ;﹏;)›` | "이제 안아주기 힘들어요... 푹 쉬다 와요" |
...
```
 ...
 수염(잔디)색은 현재 프로필 임계치에 비례합니다. 기록은 90일간 보관 후 자동 정리.

### 면도 미니게임

`promptfuzz shave`를 실행하면 먼저 **면도 방식**을 고릅니다 (난이도가 다릅니다):
...
- **⚡ 레이저** — 입력 없이 5초 후 알아서 매끈 (제일 쉬움)
- **🔌 전기면도기** — `←`/`→` 키 연타 (수염이 길수록 6→8→10번, 기존 방식)
- **🔪 날 면도기** — 화면에 뜨는 방향키(상하좌우)를 맞추기. 틀리면 *따끔!* 게이지가 한 칸 깎이고(빨강) 새 방향
- **✋ 손으로 뽑기** — 뜨는 방향키(상하좌우)를 **1초 안에**. 늦거나 틀리면 따끔 + 한 칸 깎임 + 새 방향 (제일 짜릿)

`q`로 언제든 중단(이 경우 토큰 카운터는 그대로). 비대화형 환경(Claude Code hook, CI)에서는 방식 선택 없이 자동으로 면도가 완료되고 스트레칭 카드만 한 장 출력됩니다.
...
1·5·10·30·50·100번째 면도엔 축하 박스도 함께 떠요.
