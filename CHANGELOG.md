...
- 면도 방식 4종 선택 — 레이저(5초 자동), 전기면도기(연타), 날 면도기(방향 맞추기), 손으로 뽑기(1초 타이밍). 방식마다 난이도가 다릅니다. 날 면도기·손으로 뽑기는 상하좌우 방향을 맞추고, 틀리면 따끔한 메시지와 함께 새 방향으로 다시 시도합니다. (비대화형 환경은 기존처럼 방식 선택 없이 자동 면도)
